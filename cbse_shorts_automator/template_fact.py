...
UPDATED: Bigger Fonts + Multi-line Karaoke + Aggressive Layout
"""
...
class FactTemplate:
    ...
    def generate(self, video_path, script, config, output_path):
        print("📝 Generating Fact Template (Parallel Processing)...")
        ...
        #aud_details, details_segments = karaoke_mgr.generate_timed_audio(script['fact_details'], f"{vid_id}_details", voice_key=selected_voice_key)

        print("   🎵 Generating Fact Audio...")
        
        details_path = f"{temp_dir}/{vid_id}_details.mp3"

        # Use explicit spoken text (phonetic)
        ...
        audio_files.append(f"{temp_dir}/{vid_id}_details.mp3")
        ...
        t_outro = t_cta + aud_cta.duration # NEW: Calculate precise outro start time
        
        OUTRO_DURATION = 4.0
        total_dur = t_outro + OUTRO_DURATION # CHANGED: Total duration calculated using t_outro
        ...
        # 4. Text Overlays
        clips.append(self.engine.create_text_clip("✨ DID YOU KNOW? ✨", fontsize=65, color=theme['highlight'], bold=True, stroke_color='black', stroke_width=2, align='center').set_position(('center', CARD_START_Y)).set_start(0).set_duration(total_dur))
        
        TITLE_Y = CARD_START_Y + 100
        clips.append(self.engine.create_text_clip(script['fact_title'], fontsize=60, color='white', bold=True, wrap_width=20, bg_color=hex_to_rgb(theme['bg_color']), align='North').set_position(('center', TITLE_Y)).set_start(t_title).set_duration(total_dur - t_title))
        
        # KARAOKE UPGRADE: Bigger Font, More Space
        DETAILS_Y = TITLE_Y + 250
        ...
        fact_card_clip = self.engine.create_text_clip(
            script['fact_visual'],
            fontsize=95,          # Bigger font for impact
            color=ans_text_color,
            bg_color=ans_bg_color, 
            stroke_color=stroke_col,
            stroke_width=stroke_wid, 
            bold=True,
            wrap_width=20,        # More wrapping for readability
            align='center'         # Top align for multi-line
        )

        # Display it for the exact duration of the spoken audio
        fact_card_clip = fact_card_clip.set_position(('center', DETAILS_Y)).set_start(t_details).set_duration(aud_details.duration)
        clips.append(force_rgb(fact_card_clip))
        ...
        clips.append(self.engine.create_text_clip(
            ...
            bold=True, 
            wrap_width=18
        ).set_position(('center', HEIGHT - 400)).set_start(t_cta).set_duration(cta_duration)) # CHANGED: Start is t_cta, duration is aud_cta
        
        # Outro
        outro = self.engine.create_outro(
            duration=OUTRO_DURATION,
            cta_text="SUBSCRIBE FOR FACTS!"
        ...
        audio_list = [aud_hook.set_start(t_hook), aud_title.set_start(t_title), aud_details.set_start(t_details), aud_cta.set_start(t_cta)]
        full_audio_stack = audio_list + sfx_clips # NEW: Combine Voice + SFX
